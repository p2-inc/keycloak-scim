...
public class ScimKeycloakContext extends Context {
  ...
  @Getter private final KeycloakSession keycloakSession;

  /** used to log admin events if on SCIM requests */
  @Getter private ScimAdminEventBuilder adminEventAuditer;
  ...
  public ScimKeycloakContext(KeycloakSession keycloakSession, ScimAuthorization authorization) {
    ...
    // if this consumer is called the adminEventAuditer will be overridden with an authenticated
    // instance
    /*
  ...
  /**
   * creates an admin event builder for cases in which the authentication was deactivated on a
   * specific resource endpoints. In such cases the {@link ScimAuthorization#adminAuthConsumer} will
   * not be executed and thus the authentication object is missing for the admin event builder
   */
